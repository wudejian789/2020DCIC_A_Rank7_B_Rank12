...
class TextLSTM(nn.Module):
    def __init__(self, feaSize, hiddenSize, name='textBiLSTM'):
        ...
        self.name = name
        self.LSTM = nn.LSTM(feaSize, hiddenSize, bidirectional=False, batch_first=True)
    def forward(self, x, xlen=None):
        # x: batchSizeh × seqLen × feaSize
        if xlen is not None:
            xlen, indices = torch.sort(xlen, descending=True)
        ...
        if xlen is not None:
            output, _ = nn.utils.rnn.pad_packed_sequence(output, batch_first=True)
            return output[desortedIndices]
        return output # output: batchSize × seqLen × hiddenSize

class TextBiGRU(nn.Module):
    def __init__(self, feaSize, hiddenSize, name='textBiGRU'):
        super(TextBiGRU, self).__init__()
        self.name = name
        self.biGRU = nn.GRU(feaSize, hiddenSize, bidirectional=True, batch_first=True)
    def forward(self, x, xlen=None):
        # x: batchSizeh × seqLen × feaSize
        if xlen is not None:
            xlen, indices = torch.sort(xlen, descending=True)
...
class TextGRU(nn.Module):
    def __init__(self, feaSize, hiddenSize, name='textGRU'):
        super(TextGRU, self).__init__()
        self.name = name
        self.GRU = nn.GRU(feaSize, hiddenSize, bidirectional=False, batch_first=True)
    def forward(self, x, xlen=None):
        # x: batchSizeh × seqLen × feaSize
        if xlen is not None:
            xlen, indices = torch.sort(xlen, descending=True)
            ...
            x = nn.utils.rnn.pack_padded_sequence(x[indices], xlen, batch_first=True)
        output, hn = self.GRU(x) # output: batchSize × seqLen × hiddenSize; hn: numLayers × batchSize × hiddenSize
        if xlen is not None:
            output, _ = nn.utils.rnn.pad_packed_sequence(output, batch_first=True)
            return output[desortedIndices]
        return output # output: batchSize × seqLen × hiddenSize
...
class SimpleAttention(nn.Module):
    def __init__(self, inSize, actFunc=nn.Tanh(), name='SimpleAttention', transpose=False):
